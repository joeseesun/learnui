...
- **测验模式** `/quiz/`：看标本选名字 + 看名字选标本双向四选一；连对 2 次算「掌握」，本地记录掌握度（localStorage），优先出未掌握的题，直至 86 个全部掌握；组件/风格/混合三种范围
...
- **Quiz mode** `/quiz/`: bidirectional multiple choice (specimen → name, name → specimen); an item is mastered after 2 consecutive correct answers, progress persisted locally, unmastered items asked first
