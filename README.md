...
**已验证：** 258 个静态页面零 JS 错误；62 个 UI 标本 + 44 个风格标本全部真实渲染；搜索/筛选/双语切换/复制/查词/测验交互通过；移动端无横向溢出；`python3 build.py` 一条命令重建全站。
...
- **测验模式** `/quiz/`：看标本选名字 + 看名字选标本双向四选一；连对 2 次算「掌握」，本地记录掌握度（localStorage），优先出未掌握的题，直至 106 个全部掌握；组件/风格/混合三种范围
...
cd learnui
python3 build.py          # 生成 site/（152 页 + 106 个标本嵌入页）
python3 scripts/gen-og.py # 生成 150 张 og 分享图 → assets/og/（需 playwright）
...
- 全量 258 页 Playwright 巡检：无 JS 错误，标本均有真实渲染尺寸
...
git clone https://github.com/joeseesun/learnui.git
cd learnui
python3 build.py          # builds site/ (152 pages + 106 specimen embeds, stdlib only)
...
- All 258 pages load with zero JS errors; every specimen renders with real dimensions
