...
> UI 视觉词典的中英双语对照版：看到元素，学会它的真名（Web CSS/ARIA + macOS AppKit/SwiftUI），然后精准地指挥你的 AI 编程代理。
> A bilingual (EN/中文) visual dictionary of UI: see the element, learn its real name, and prompt your coding agent with precision.

[在线演示 Live Demo](https://leanui.qiaomu.ai/) · [词条示例](https://leanui.qiaomu.ai/web/text-scramble/) · [翻译对照表](https://leanui.qiaomu.ai/guides/translate/)

**已验证：** 66 个静态页面零 JS 错误；62 个标本全部真实渲染；搜索/筛选/双语切换/复制/查词交互通过；移动端无横向溢出；`python3 build.py` 一条命令重建全站。

## 这是什么

[namethatui.com](https://namethatui.com/) 的**完整内容复刻 + 中英双语对照版**，为学习目的而建。原站是「UI 视觉词典」——每个词条给出一个活的界面标本、它的正式名称（Web 的 CSS/ARIA 叫法、macOS 的 AppKit/SwiftUI 叫法）、一句人话解释，和一段可以直接喂给 AI 编程代理的 prompt。
...
- **全站搜索**：支持中英文模糊描述（试试「三个点」「mac 窗口按钮」）
- **双击查词**：双击任意英文单词，弹出通俗英文释义
- **一键复制**：Prompt、调试 Prompt、整页 Markdown（直接粘贴给你的 AI 代理）

## 为什么值得用

给 AI 编程代理写 prompt 时，最大的损耗是「那个东西叫什么」。知道 *scrim*、*disclosure triangle*、*token field* 这些真名，代理一次就能改对地方。双语对照让中文读者不用再猜英文术语对应什么。
...
│   └── ui.json         # 站点文案（双语）
├── demos/<slug>.html   # 62 个自包含交互标本（scoped CSS + 内联 JS）
├── assets/             # site.css / site.js / 自托管 Geist 字体
├── DESIGN.md           # 设计系统锚点（暖纸标本馆风格）
└── site/               # 构建产物（git 忽略，build.py 生成）
```

## 设计

「暖纸标本馆」：暖石纸底、发丝边框、信号橙单一重音、Geist/Geist Mono 自托管字体。标本如实还原被模仿系统的外观（macOS Sonoma 观感、Web 白卡片风），站点 chrome 保持克制。完整设计约束见 [DESIGN.md](DESIGN.md)。

## 实测验证

- 全量 66 页 Playwright 巡检：无 JS 错误，62 个标本均有真实渲染尺寸
- 交互测试：中英文搜索、平台筛选、语言三态切换、随机词条、剪贴板复制、翻译表过滤、灯箱开合，全部通过
- 移动端 390px 无横向溢出（翻译表独立横向滚动）
- 线上环境：<https://leanui.qiaomu.ai/> 200，HTTPS + HSTS，Umami 统计链路实测写入成功

## 限制与版权

- 英文源内容（`data/entries.json`、`data/guides.json`）复刻自 [namethatui.com](https://namethatui.com/)，版权归原作者；本仓库代码（构建器、标本重实现、样式、译文）以 [MIT](LICENSE) 开源。
...
# name that ui? — Bilingual (EN/中文) Visual Dictionary of UI

A faithful content replica of [namethatui.com](https://namethatui.com/) with full Chinese-English parallel text, built for learning. See a UI element, learn its real name (Web CSS/ARIA + macOS AppKit/SwiftUI), and prompt your coding agent with precision.

**Live demo:** <https://leanui.qiaomu.ai/>
...
- **Full-text search** in English and Chinese, **double-click any word** for a plain-English definition, **copy page as Markdown**
...
git clone https://github.com/joeseesun/learnui.git
cd learnui
python3 build.py          # builds site/ (66 pages, stdlib only)
cd site && python3 -m http.server 8000
```
...
- All 66 pages load with zero JS errors; every specimen renders with real dimensions
